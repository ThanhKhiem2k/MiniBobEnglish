...
const WordType5: React.FC<IWordType3Props> = ({
  offsets,
  index,
  children,
  selected,
  ...
  const startY = useSharedValue<number>(offset.originalY.value + MARGIN_TOP_TYPES);
        ...
        if (
          ...
          for (let i = 0; i < len; i++) {
            ...
            if (positionY.length > 1) {
              if (
                translation.y.value < (positionY[i] - WORD_HEIGHT - 20) * -1 &&
                (translation.y.value > (positionY[i + 1] - WORD_HEIGHT) * -1 || i == len - 1) &&
                positionX[len - i - 1] - widthLine - marginLeftText <
                translation.x.value &&
                positionX[len - i - 1] + widthLine + marginLeftText >
                translation.x.value
              ) {
  ...
  return (
    <View>
      ...
      <Animated.View style={style}>
        <GestureDetector gesture={!Review ? dragGesture : undefined}>
